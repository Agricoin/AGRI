...
void SendCoinsDialog::updateGlobalFeeVariables()
{
    if (ui->radioSmartFee->isChecked())
    {
        ...
        ui->confirmationTargetLabel->setText(GUIUtil::formatDurationStr(nConfirmTarget * (Params().GetConsensus().nTargetTimespan / 2)) + " / " + tr("%n block(s)", "", nConfirmTarget));
    }
    else
    {
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid AGRICOIN address"));
        }
        else // Valid address
        {
            CKeyID keyid;
